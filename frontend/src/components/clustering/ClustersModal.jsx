    ...
    >
      <div className="modal-content" style={{ maxWidth: '750px' }}>
        ...
        >
          ...
          Todos los Grupos de Tendencias ({clusters.length} Clusters K-Means)
        </h2>
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '1.5rem' }}>
          El modelo no supervisado K-Means agrupa automáticamente el conocimiento por similitud semántica de vectores (TF-IDF).
        </p>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          {clusters.map((cluster) => (
            <div key={cluster.id} className="cluster-folder" style={{ marginBottom: 0 }}>
              <div>
                <h4 style={{ color: 'var(--text-primary)', fontSize: '0.95rem' }}>{cluster.title}</h4>
                <span style={{ color: 'var(--text-secondary)', fontSize: '0.8rem' }}>
                  {cluster.docsCount} docs • tags: {cluster.tags}
                </span>
              </div>
              <svg className="icon" viewBox="0 0 24 24">
